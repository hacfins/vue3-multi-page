<template>
    <div class="page-title-wrapper">
        <span class="page-title">系统概况专题</span>
    </div>
        ...
        <Panel>
            ...
            <span class="square-bg">
              ...
              <i class="num">{{ union.deptCount }}</i><i class="white-font">（个）</i>
                    ...
                    <chart ref="chart1" autoresize :option="returnPie(union.chartData)"
            ...
            <span class="square-bg data-change">
              ...
              <span>
                ...
                <i>归集数据量</i><br>
                <i style="line-height: 40px;">正确率</i><i
                  style="color: #0cf; line-height: 40px;">&nbsp;{{ dataExchange.getInCorrect }}</i><br>
              </span>
              <span>
                <i class="num">{{ dataExchange.getOut }}</i><i>（条）</i><br>
                <i>输出数据量</i><br>
                <i style="line-height: 40px;">正确率</i><i
                  style="color: #0cf; line-height: 40px;">&nbsp;{{ dataExchange.getOutCorrect }}</i><br>
              </span>
            </span>
              ...
              <span style="padding-bottom: 0; padding-top: 0;">
                ...
                <i style="line-height: 40px;">正常任务</i>&nbsp;<i class="num">{{ dataExchange.dockedNormal }}</i>&nbsp;<i
                  style="line-height: 40px;">异常</i>&nbsp;<i class="num">{{ dataExchange.dockedAbnormal }}</i>
                    ...
                    <chart ref="chart2" autoresize :option="returnBar(dataExchange.chartData)"
        ...
        <Panel>
            <template #inner>
                <div style="height: 100%" flex="main:justify dir:top">
                    ...
                    <chart ref="chart2" autoresize :option="returnCollection(collection.chartData)"
                           style="width: 100%; height: 220px;display:block;"></chart>
              ...
              <i class="btn" :class="handleBtnTab2 === 1 ? 'active' : ''" @click="changeReportDataYear">本年</i>
                    ...
                    <chart ref="chart2" autoresize :option="returnArea(report.chartData)"
                           style="width: 100%; height: 220px;display:block;"></chart>
    ...
    export default {
        ...
        created() {
            ...
            this.$store.dispatch('getOverviewAggregateData').then(() => {
                this.numChange(this.$store.state.total)
            })
        },
        mounted() {
        },
        computed  : {
            ...mapState({
                ...
                dataExchange: state => state.dataExchange,
                total       : state => state.total,
                rotateData  : state => state.rotateData,
                redBlackList: state => state.redBlackList,
...
<style lang="scss" rel="stylesheet/scss" scoped>
    ...
    .page-title {
        background-image        : -webkit-linear-gradient(0, #1fb9fc, #1d80d2, #1850a7);
    ...
    @-webkit-keyframes rotate3dOpposite {
        0% {
            transform : rotateZ(0);
        }
        100% {
    ...
    @-moz-keyframes rotate3dOpposite {
        0% {
            transform : rotateZ(0);
        }
        100% {
